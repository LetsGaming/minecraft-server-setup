...
const VARIABLES_PATH = path.resolve(SCRIPT_DIR, '..', 'common', 'variables.txt');
const DOWNLOADED_VERSIONS_PATH = path.resolve(SCRIPT_DIR, '..', 'common', 'downloaded_versions.json');
...
function writeDownloadedVersions(data) {
  fs.writeFileSync(DOWNLOADED_VERSIONS_PATH, JSON.stringify(data, null, 2), 'utf8');
}
...
async function getVersionInfo(requestedVersion, allowSnapshot = false) {
  ...
  const versionId = requestedVersion === 'latest'
    ? (allowSnapshot ? manifest.latest.snapshot : manifest.latest.release)
    : requestedVersion;
...
async function getJavaVersionFor(mcVersion) {
  ...
  if (meta.javaVersion?.majorVersion) return String(meta.javaVersion.majorVersion);
  console.warn(`No javaVersion in Mojang metadata for ${mcVersion}. Falling back to Java 8.`);
  return '8';
}
...
function askYesNo(question) {
  return new Promise(resolve => {
    ...
    rl.question(`${question} (y/n): `, answer => { rl.close(); resolve(answer.toLowerCase().startsWith('y')); });
  });
}
...
function runCheckUpdatesJSON(version) {
  return new Promise((resolve, reject) => {
    execFile('node', [CHECK_UPDATES_SCRIPT, version, '--json'], (err, stdout) => {
      if (err) return reject(err);
      try { resolve(JSON.parse(stdout)); }
      catch { reject(new Error('Failed to parse JSON from check-updates.js')); }
    });
  });
}
...
function backupServer() {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn('bash', [BACKUP_SCRIPT, '--archive', 'update'], { stdio: 'inherit' });
    child.on('exit', code => {
...
async function ensureJavaVersion(mcVersion) {
  ...
  if (installed) { console.log(`Java ${requiredJava} already installed (${installed}).`); return requiredJava; }
  ...
  try {
    ...
    const candidates = listOutput.split('\n').map(l => l.trim())
      .filter(l => l.startsWith(`adopt@${requiredJava}.`) || l.startsWith(`temurin@${requiredJava}.`))
      .sort().reverse();
    if (!candidates.length) throw new Error(`No Jabba candidates for Java ${requiredJava}. Install manually.`);
...
async function updateFabric(versionId, serverPath) {
  ...
  const info = await axios.get('https://meta.fabricmc.net/v2/versions/installer');
  ...
  const jabbaDir = path.join(process.env.HOME, '.jabba', 'jdk');
  const javaInstall = fs.readdirSync(jabbaDir).find(n => n.includes(`@${javaVersion}.`));
  if (!javaInstall) throw new Error(`Java ${javaVersion} not found after install attempt.`);
  ...
  await new Promise((resolve, reject) => {
    ...
    proc.on('exit', code => {
      ...
      code === 0 ? resolve() : reject(new Error(`Fabric installer exit ${code}`));
    });
  });
...
function runModUpdate(version) {
  return new Promise((resolve, reject) => {
    const child = spawn('node', [UPDATE_MODS_SCRIPT, version], { stdio: 'inherit' });
    child.on('exit', code => code === 0 ? resolve() : reject(new Error('update-mods.js failed')));
  });
}
...
(async () => {
  ...
  try {
    ...
    const { versionId, metadataUrl } = await getVersionInfo(targetVersionArg || 'latest');
    ...
    if (incompatible.length > 0) {
      ...
      removeIncompatibleModsFlag = await askYesNo('Remove incompatible mods from the server?');
    }
