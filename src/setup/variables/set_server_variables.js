...
function buildJavaArgs(config) {
  ...
  if (config.metaspaceLimit)
    flags.push(`-XX:MaxMetaspaceSize=${config.metaspaceLimit}`);

  const gcChoice = config.garbageCollector;
  if (!gcChoice || !["g1gc", "zgc"].includes(gcChoice)) {
    throw new Error(
      "Invalid or missing garbageCollector (must be 'g1gc' or 'zgc').",
    );
  }
  ...
  if (gcChoice === "g1gc") {
    ...
    if (g1gc.g1NewSizePercent)
      experimentalFlags.push(`-XX:G1NewSizePercent=${g1gc.g1NewSizePercent}`);
    if (g1gc.g1MaxNewSizePercent)
      experimentalFlags.push(
        `-XX:G1MaxNewSizePercent=${g1gc.g1MaxNewSizePercent}`,
      );
    ...
    if (g1gc.maxPauseMillis)
      flags.push(`-XX:MaxGCPauseMillis=${g1gc.maxPauseMillis}`);
    if (g1gc.heapRegionSize)
      flags.push(`-XX:G1HeapRegionSize=${g1gc.heapRegionSize}`);
    if (g1gc.reservePercent)
      flags.push(`-XX:G1ReservePercent=${g1gc.reservePercent}`);
    if (g1gc.heapWastePercent)
      flags.push(`-XX:G1HeapWastePercent=${g1gc.heapWastePercent}`);
    if (g1gc.mixedGCCountTarget)
      flags.push(`-XX:G1MixedGCCountTarget=${g1gc.mixedGCCountTarget}`);
    if (g1gc.initiatingHeapOccupancyPercent)
      flags.push(
        `-XX:InitiatingHeapOccupancyPercent=${g1gc.initiatingHeapOccupancyPercent}`,
      );
    if (g1gc.survivorRatio)
      flags.push(`-XX:SurvivorRatio=${g1gc.survivorRatio}`);
    ...
    if (zgc.uncommitDelay)
      flags.push(`-XX:ZUncommitDelay=${zgc.uncommitDelay}`);
    if (zgc.uncommitDelayOnIdle)
      flags.push(`-XX:ZUncommitDelayOnIdle=${zgc.uncommitDelayOnIdle}`);
    if (zgc.heapReservePercent)
      flags.push(`-XX:ZHeapReservePercent=${zgc.heapReservePercent}`);
    if (zgc.concurrentGCThreads)
      flags.push(`-XX:ConcGCThreads=${zgc.concurrentGCThreads}`);
    if (zgc.softMaxHeapSize)
      flags.push(`-XX:SoftMaxHeapSize=${zgc.softMaxHeapSize}`);
  }
